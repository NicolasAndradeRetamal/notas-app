...
export function Sidebar({ notesTotal, trashTotal, notebooks, tags }: SidebarProps) {
  return (
    <nav aria-label="Cuadernos y etiquetas" className="flex h-full flex-col gap-6 overflow-y-auto p-3">
      <ul className="space-y-0.5">
        <li>
          <SidebarNavLink href="/notes" count={notesTotal} icon={<FileText className="h-4 w-4 shrink-0" aria-hidden="true" />}>
            Todas las notas
          </SidebarNavLink>
        </li>
        <li>
          <SidebarNavLink href="/trash" count={trashTotal} icon={<Trash2 className="h-4 w-4 shrink-0" aria-hidden="true" />}>
            Papelera
          </SidebarNavLink>
        </li>
        <li>
          <span className="flex h-10 items-center gap-3 rounded-md px-3 text-sm text-ink-subtle opacity-70">
